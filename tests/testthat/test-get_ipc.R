test_that("get_ipc works", {
  ...
  testthat::expect_equal(nrow(cba_cbna_r), 234)
  ipab <- get_ipab(folder = tempdir(), sheet = 1)
  testthat::expect_equal(nrow(ipab), 226)
  df <- basket_goods(data = ech::cba_cbna_mdeo, year = 2018)
  testthat::expect_equal(nrow(df), 234)
  df <- basket_goods(data = ech::cba_cbna_int, year = 2018)
  testthat::expect_equal(nrow(df), 234)
  df <- basket_goods(data = ech::cba_cbna_rur, year = 2018)
  testthat::expect_equal(nrow(df), 234)
})
