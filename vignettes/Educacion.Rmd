...
Estimamos la proporción de personas que asisten actualmente a un centro educativo formal entre personas de .......:

```{r eval = FALSE}
a <- get_estimation_mean(df, variable = "school_enrollment", by.x = "dpto", domain = "age_groups == 3", level = "i")
get_estimation_mean(df, variable = "school_enrollment", domain = "age_groups == 3", level = "i")
```

Estimamos los años de educación según sexo

```{r eval = FALSE}
get_estimation_mean(df, variable = "years_schooling", by.x = "e26", domain = "age_groups == 5", level = "i")
...
```{r eval = FALSE}
get_estimation_mean(df, variable = "level_education", by.x = "dpto", level = "i")
```
