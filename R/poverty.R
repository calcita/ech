  ...
  if (yy < 2016) {
    data <- data %>%
      dplyr::mutate(
        ...
        UBN_q = UBN_housing + UBN_water + UBN_sewerage + UBN_electricity + UBN_education,
        UBN = dplyr::case_when(
          UBN_q == 0 ~ 0,
          UBN_q == 1 ~ 1,
          UBN_q == 2 ~ 2,
    ...
    data <- data %>%
      dplyr::mutate(
        ...
        UBN_education = ifelse(e27 %in% 4:17 & (.data$school_enrollment == 0 | .data$e238 == 2) & .data$years_schooling < 12, 1, 0),
