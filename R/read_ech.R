...
#' @param formato si no se indica se descarga en formato .sav (SPSS), la otra
#' opción sería "dat"
...
download_ech <- function(anios = NULL, carpeta = getwd(), formato = "sav") {
  # checks ----
  ...
  if (!is.null(anios) & any(anios %in% all_years) == FALSE) {
    stop("Por el momento solo funciona con la ECH entre los anios 2011 y 2017")
  }
  ...
  urls <- data.frame(cbind(year = 2011:2018,
                           ...
                           md_dat = c( "http://www.ine.gub.uy/c/document_library/get_file?uuid=7eecaefc-be12-4f5f-93a4-d53cc2a20070&groupId=10181",
                                       ...
                                       "http://www.ine.gub.uy/c/document_library/get_file?uuid=32a696cf-38d9-46cb-ad16-8fea69262581&groupId=10181"),
                           dic = c("http://www.ine.gub.uy/c/document_library/get_file?uuid=54523778-5f53-4df1-a265-3ff520941bca&groupId=10181",
                                   ...
                                   "http://www.ine.gub.uy/c/document_library/get_file?uuid=73b6cc21-1bb0-483b-a463-819315b5fff3&groupId=10181"),
                           file = paste0(carpeta, "/ech_", all_years, "_spss.rar")),
                     stringsAsFactors = FALSE)
  links <- urls[urls$year %in% anios, ]

  for (j in 1:nrow(links)) {
    u <- links[ifelse(formato == "sav", 2, 3)][[j]] # parametrizar con formato
    f <- links$file[[j]]
    y <- links$year[[j]]
...
#' @param anios si no se indica un anio, descarga todos los anios disponibles
#' @param carpeta si no se indica una carpeta, descarga en la carpeta temp
#' @param formato si no se indica se descarga en formato .sav (SPSS), la otra
#' opción sería "dat"
#' @importFrom utils download.file
#' @importFrom glue glue
...
get_ech <- function(anios = NULL, carpeta = tempdir(), formato = "sav"){
